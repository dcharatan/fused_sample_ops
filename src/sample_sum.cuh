...
__launch_bounds__(256) __global__ void sample_sum_backward_kernel(
  ...
  //       atomicAdd(&image_gradients[b][c][iy_nw][ix_nw], output_gradient * nw *
  //       query); const scalar_t pixel_nw_query = pixel_nw * query; sample_gradient_x
  //       -= pixel_nw_query * (iy_se - iy); sample_gradient_y -= pixel_nw_query *
  //       (ix_se - ix);
  //     }
  //     if (within_bounds_2d(iy_ne, ix_ne, H, W)) {
  //       const scalar_t pixel_ne = images[b][c][iy_ne][ix_ne];
  //       query_gradient += pixel_ne * ne;
  //       atomicAdd(&image_gradients[b][c][iy_ne][ix_ne], output_gradient * ne *
  //       query); const scalar_t pixel_ne_query = pixel_ne * query; sample_gradient_x
  //       += pixel_ne_query * (iy_sw - iy); sample_gradient_y -= pixel_ne_query * (ix -
  //       ix_sw);
  //     }
  //     if (within_bounds_2d(iy_sw, ix_sw, H, W)) {
  //       const scalar_t pixel_sw = images[b][c][iy_sw][ix_sw];
  //       query_gradient += pixel_sw * sw;
  //       atomicAdd(&image_gradients[b][c][iy_sw][ix_sw], output_gradient * sw *
  //       query); const scalar_t pixel_sw_query = pixel_sw * query; sample_gradient_x
  //       -= pixel_sw_query * (iy - iy_ne); sample_gradient_y += pixel_sw_query *
  //       (ix_ne - ix);
  //     }
  //     if (within_bounds_2d(iy_se, ix_se, H, W)) {
  //       const scalar_t pixel_se = images[b][c][iy_se][ix_se];
  //       query_gradient += pixel_se * se;
  //       atomicAdd(&image_gradients[b][c][iy_se][ix_se], output_gradient * se *
  //       query); const scalar_t pixel_se_query = pixel_se * query; sample_gradient_x
  //       += pixel_se_query * (iy - iy_nw); sample_gradient_y += pixel_se_query * (ix -
  //       ix_nw);
  //     }
