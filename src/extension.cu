...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("sample_dot_forward", &fused_grid_ops::sample_dot_forward,
        "fused grid sample and dot product forward (CUDA)");
  m.def("sample_dot_backward", &fused_grid_ops::sample_dot_backward,
        "fused grid sample and dot product backward (CUDA)");
  m.def("sample_sum_forward", &fused_grid_ops::sample_sum_forward,
        "fused grid sample and sum forward (CUDA)");
  m.def("sample_sum_backward", &fused_grid_ops::sample_sum_backward,
