...
void fused_grid_sum::backward(torch::Tensor result_gradients,
                              torch::Tensor images,
  ...
  // Compute the number of threads. For now, just parallelize over independent samples.
  const int64_t b = result_gradients.size(0);  // batch
  const int64_t hd = result_gradients.size(1);  // head
  const int64_t s = result_gradients.size(2);  // sample_independent
  const int64_t c = result_gradients.size(3);  // channel
  const int64_t num_threads = b * hd * s * c;

  const int grid_size = (num_threads + BLOCK_SIZE - 1) / BLOCK_SIZE;
  AT_DISPATCH_FLOATING_TYPES(
      samples.scalar_type(), "backward_kernel", ([&] {
        backward_kernel<scalar_t><<<grid_size, BLOCK_SIZE>>>(
            result_gradients.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
            images.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
            image_gradients.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
            samples.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
            weights.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
            weight_gradients
